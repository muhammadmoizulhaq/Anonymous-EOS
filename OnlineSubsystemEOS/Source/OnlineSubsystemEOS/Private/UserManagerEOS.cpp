...
typedef TEOSCallback<EOS_Connect_OnCreateDeviceIdCallback, EOS_Connect_CreateDeviceIdCallbackInfo> FConnectCreateDeviceIdCallback;
...
///////////////////////////////////
// Start custom plugin logic
///////////////////////////////////

/**
 * @brief Start the anonymous login. First clears the device that is stored in the system.
 */
void FUserManagerEOS::StartAnonLogin(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	UE_LOG_ONLINE(Display, TEXT("StartAnonLogin called => Finding and/or deleting existing devices."));
	...
	FCallbackBase::DoNotCancelAllCallbacks();
	...
	{
		UE_LOG_ONLINE(Display, TEXT("StartAnonLogin Lambda"));

		if (Data->ResultCode == EOS_EResult::EOS_Success || Data->ResultCode == EOS_EResult::EOS_NotFound)
		{
			UE_LOG_ONLINE(Display, TEXT("StartAnonLogin: EOS_Sucess => Device deleted! Or no existing device has been found! Code: (%s)"), ANSI_TO_TCHAR(EOS_EResult_ToString(Data->ResultCode)));
			CreateAnonDevice(LocalUserNum, AccountCredentials);
		}
		else
		{
			UE_LOG_ONLINE(Error, TEXT("StartAnonLogin failed with EOS result code (%s)"), ANSI_TO_TCHAR(EOS_EResult_ToString(Data->ResultCode)));
		}
	};

	UE_LOG_ONLINE(Display, TEXT("Going to call EOS_Connect_DeleteDeviceID()"));
	EOS_Connect_DeleteDeviceId(EOSSubsystem->ConnectHandle, &DeleteDeviceIdOptions, CallbackObj, CallbackObj->GetCallbackPtr());
}

/**
 *	@brief Function that creates a new anon device.
 */
void FUserManagerEOS::CreateAnonDevice(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	UE_LOG_ONLINE(Display, TEXT("CreateAnonDevice called => Creating a new device."));

	// Device options. This will help creating the Device ID.
	...
	CreateDeviceIdOptions.DeviceModel = "PC Windows";

	UE_LOG_ONLINE(Display, TEXT("CreateAnonDevice: Starting Create Device Id."));
	// Create the Device ID. We will use this ID to connect to EOS.

	FConnectCreateDeviceIdCallback* CallbackObj = new FConnectCreateDeviceIdCallback();
	CallbackObj->CallbackLambda = [this, LocalUserNum, AccountCredentials](const EOS_Connect_CreateDeviceIdCallbackInfo* Data)
	{
		UE_LOG_ONLINE(Display, TEXT("CreateAnonDevice Lambda"));

		if (Data->ResultCode == EOS_EResult::EOS_Success)
		{
			UE_LOG_ONLINE(Display, TEXT("CreateAnonDevice: EOS_Success => Created a new device with success!"));
			LoginAnonDevice(LocalUserNum, AccountCredentials);
		}
		else
		{
			UE_LOG_ONLINE(Error, TEXT("CreateAnonDevice failed with EOS result code (%s)"), ANSI_TO_TCHAR(EOS_EResult_ToString(Data->ResultCode)));
		}
	};
	
	EOS_Connect_CreateDeviceId(EOSSubsystem->ConnectHandle, &CreateDeviceIdOptions, CallbackObj, CallbackObj->GetCallbackPtr());
}

/**
 * @brief Function to login the anonymous device.
 */
void FUserManagerEOS::LoginAnonDevice(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	UE_LOG_ONLINE(Display, TEXT("LoginAnonDevice called => Logging the device in."));
	...
	UE_LOG_ONLINE(Display, TEXT("LoginAnonDevice: Starting login process."));
	
	FConnectLoginCallback* LoginCallback = new FConnectLoginCallback();
	LoginCallback->CallbackLambda = [this, LocalUserNum](const EOS_Connect_LoginCallbackInfo* Data)
	{
		if (Data->ResultCode == EOS_EResult::EOS_Success)
		{
			UE_LOG_ONLINE(Display, TEXT("LoginAnonDevice: EOS_Success => Could login with the device ID."));

			CompleteAnonLogin(LocalUserNum, nullptr, Data->LocalUserId);
			
			//UE_LOG_ONLINE(Display, TEXT("LoginAnonDevice: Calling Login Complete delegates and status change delegates."));
			//TriggerOnLoginCompleteDelegates(LocalUserNum, true, *FUniqueNetIdEOS::EmptyId(), FString());
			//TriggerOnLoginStatusChangedDelegates(LocalUserNum, ELoginStatus::NotLoggedIn, ELoginStatus::LoggedIn, *FUniqueNetIdEOS::EmptyId());
		}
		else
		{
			UE_LOG_ONLINE(Error, TEXT("LoginAnonDevice failed with EOS result code (%s)"), ANSI_TO_TCHAR(EOS_EResult_ToString(Data->ResultCode)));
		}
	};
	EOS_Connect_Login(EOSSubsystem->ConnectHandle, &LoginOptions, LoginCallback, LoginCallback->GetCallbackPtr());
}

/**
 * @brief Completes the login of the user.
 */
void FUserManagerEOS::CompleteAnonLogin(int32 LocalUserNum, EOS_EpicAccountId AccountId, EOS_ProductUserId UserId)
{
		UE_LOG_ONLINE(Display, TEXT("CompleteAnonLogin"));
	if (LoginNotificationId == 0)
	{
		UE_LOG_ONLINE(Display, TEXT("LoginNotificationId == 0"));
		...
		UE_LOG_ONLINE(Display, TEXT("Notif ID: %llu"), LoginNotificationId);
	}

	if (!LocalUserNumToConnectLoginNotifcationMap.Contains(LocalUserNum))
	{
		UE_LOG_ONLINE(Display, TEXT("!LocalUserNumToConnectLoginNotifcationMap.Contains(LocalUserNum)"));
		...
		{
			UE_LOG_ONLINE(Display, TEXT("FRefreshAuthCallback Lambda"));
	...
	UE_LOG_ONLINE(Display, TEXT("UserNetId is valid!"));
...
bool FUserManagerEOS::Login(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	...
	if (Settings.bOverrideAuthLoginWithConnectLogin)
	{
		UE_LOG_ONLINE(Display, TEXT("bOverrideAuthLoginWithConnectLogin == true"));
		StartAnonLogin(LocalUserNum, AccountCredentials);
		return true;
	}
...
bool FUserManagerEOS::Logout(int32 LocalUserNum)
{
	...
	if (Settings.bOverrideAuthLoginWithConnectLogin)
	{
		...
		UE_LOG_ONLINE(Display, TEXT("Logout!!"));
		return true;
	}
